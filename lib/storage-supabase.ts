...
// ===== FULL DATABASE STORAGE (SUPABASE ONLY) =====
...
export async function loadState(): Promise<AppState> {
  ...
  try {
    ...
    const { error } = await supabase
        ...
        .upsert({
            user_id: userId,
            data: updated as any, // Ini sudah benar
            updated_at: new Date().toISOString(),
        } as any, { // Tambahkan 'as any' di sini untuk membungkam error TS sementara
            onConflict: 'user_id'
        })

    if (!error && data && (data as { data?: AppState }).data) {
        const state = { ...DEFAULT_STATE, ...((data as { data?: AppState }).data) }
        stateCache = state
        lastFetch = Date.now()
        return state
    }
